...
let embed = new MessageEmbed().setAuthor(message.member.displayName, message.author.avatarURL({display: true})).setColor("RANDOM")
   ...
   var money = Math.floor(Math.random() * 40000) + 5000
    let balance = coinData.coin
 

    let timeout = 1000*60*60*24
    let bump = await qdb.get(`cooldowns.daily.${message.author.id}`)
    ...
    message.channel.send(`**:moneybag: ${message.author.username},** senin  günlük **${client.emoji("exzowoncy")} ${money} exzowoncy** paran burada!`)
    await qdb.set(`cooldowns.daily.${message.author.id}`, Date.now())
};
