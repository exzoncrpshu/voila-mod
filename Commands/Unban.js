...
module.exports.execute = async (client, message, args, ayar, emoji) => {
  ...
  if(!ayar.banciRolleri.some(rol => message.member.roles.cache.has(rol)) && !message.member.roles.cache.has(ayar.sahipRolu)) return message.channel.csend(embed.setDescription("Bu komutu kullanabilmek için gerekli rollere sahip değilsin!")).then(x => x.delete({timeout: 5000}));
  ...
  if(kisi) {
    ...
    message.channel.send(embed.setDescription(`\`${kisi.tag}\` kişisinin banı kaldırıldı!`))
    if(ayar.banLogKanali && client.channels.cache.has(ayar.banLogKanali)) client.channels.cache.get(ayar.banLogKanali).send(new MessageEmbed().setColor("GREEN").setDescription(`${message.author} yetkilisi ban kaldırdı!\n\n• Yasağı Kaldırılan Üye: ${kisi.tag} (\`${kisi.id}\`)\n• Yasağı Kaldıran Yetkili: ${message.author} (\`${message.author.tag}\`- \`${message.author.id}\`)`));
  } else {
