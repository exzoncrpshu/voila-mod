...
let embed = new MessageEmbed().setAuthor(message.member.displayName, message.author.avatarURL({display: true})).setColor("RANDOM")
  ...
  if (args[0] === "ekle" || args[0] === "add") {
    ...
    await coin.findOneAndUpdate({ guildID: message.guild.id, userID: member.id }, { $inc: { coin: count } }, { upsert: true });
    const coinData = await coin.findOne({ guildID: message.guild.id, userID: member.id });
    let addedRoles = "";
...
module.exports.configuration = {
    name: "puan",
    aliases: ["coin", "xp"],
    usage: "coin ekle/sil/gönder [üye] [miktar]",
