...
let embed = new MessageEmbed().setAuthor(message.member.displayName, message.author.avatarURL({display: true})).setColor("RANDOM")
   ...
   var money = Math.floor(Math.random() * 150000) + 50000
    let balance = coinData.coin
 

    let timeout = 1000*60*60*24*28
    let bump = await qdb.get(`cooldowns.monthly.${message.author.id}`)
    ...
    message.channel.send(`**:moneybag: ${message.author.username},** senin  aylık **${client.emoji("exzowoncy")} ${money} exzowoncy** paran burada!`)
    await qdb.set(`cooldowns.monthly.${message.author.id}`, Date.now())
};
