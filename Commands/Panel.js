...
module.exports.execute = async (client, message, args) => {
  ...
  let ozellikler = [
    ...
    { name: "yetkiliAlim", type: "roller"},
    { name: "kayitLog", type: "kanal" },
    ...
    { name: "voiceMuteLogKanali", type: "kanal" }
  ];
  let secim = args[0];
  ...
  if (secim === "sıfırla") {
    if (!args[1] || (args[1] !== "ses" && args[1] !== "chat")) return message.channel.csend(embed.setDescription("Sıfırlanacak veriyi belirtmelisin! (ses/chat/teyit)")).then(x => x.delete({timeout: 5000}));
    if (args[1] === "ses") {
      idb.delete("sesli");
    } else if (args[1] === "chat") {
      ldb.delete("kanallar");
  ...
  if (ozellik.type == "tekil"){
    ...
    if(roller.length <= 0) return message.channel.csend(embed.setDescription("**Rol belirtmeyi unuttun!**"));
    ...
    if(!rol) return message.channel.csend(embed.setDescription("**Rol belirtmeyi unuttun!**"));
    ...
    if(!channel) return message.channel.csend(embed.setDescription("**Kanal belirtmeyi unuttun!**"));
