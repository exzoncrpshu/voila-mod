...
module.exports = (oldUser, newUser) => {
  ...
  if(!user) return;
  let embedd = new discord.MessageEmbed().setAuthor(guild.name, guild.iconURL({dynamic: true})).setColor(client.randomColor());
  const embed = new discord.MessageEmbed().setAuthor(user.displayName, user.user.avatarURL({dynamic: true})).setFooter("VOILA ❤️ EXZONCRP").setColor(client.randomColor()).setTimestamp();
  let log = client.channels.cache.get(ayarlar.ekipLogKanali);
  let yasakTaglilar = db.get('yasakTaglilar') || [];
  let chat = client.channels.cache.get(ayarlar.chatKanali);
  ...
  if(newUser.username.includes(ayarlar.tag) && !user.roles.cache.has(ayarlar.ekipRolu)){
      ...
      if(ayarlar.chatKanali && chat) chat.send(`${user}`, {embed: embedd.setDescription(`${user} üyesi tagımızı ismine alarak ailemize katıldı!`)}).catch();
      ...
      if(ayarlar.chatKanali && chat) chat.send(`${user}`, {embed: embedd.setDescription(`${user} üyesi tagımızı isminden çıkararak ailemizden ayrıldı!`)}).catch();
    }
}
